...
void HistosCombine()
{
  ...
  cout << "Starting recursions. These may take a few seconds" << endl
       << flush;
  ...
  for (auto itrg = 0u; itrg != jp::notrigs; ++itrg)
  {
    ...
    if (jp::ismc and jp::usemctrig)
    { // When in mc, we need to know how to use trigger weighting. Either mc or triggers are ignored
  ...
  // if (jp::isdt)
  //   recurseFile(fin, fout, "hlumi_orig");
  ...
  recurseFile(fin, fout, "hdjRMmass", false);
  recurseFile(fin, fout, "hdjmass_half", false);
  recurseFile(fin, fout, "hdjRMmass_half", false);
  ...
  recurseFile(fin, fout, "hdjRMmass_gen", false);
  recurseFile(fin, fout, "hdjmass_half_gen", false);
  recurseFile(fin, fout, "hdjRMmass_half_gen", false);
...
{                                                                               // Copying a TProfile bin is complicated
...
inline void fillHisto(T *hpt, TDirectory *outdir, TDirectory *indir, bool ptSelect, bool othProf)
{
  ...
  if (ptSelect)
  {
    ...
    for (int i = 1; i != hpt->GetNbinsX() + 1; ++i)
    {
      ...
      { // TODO: We could do better than a linear search!
        if (isth23)
        { // TH2 and TH3
...
void recurseFile(TDirectory *indir, TDirectory *outdir, string hname, bool ptSelect, bool othProf, int lvl, double etamid)
{
  ...
  if (lvl < 2)
  {
    ...
    while ((key = dynamic_cast<TKey *>(itkey.Next())))
    {
      ...
      { // All keys are looped over only in search for subdirs
        obj = key->ReadObj();
        assert(obj);
      }
      else
      ...
      if (obj->InheritsFrom("TDirectory"))
      {
        ...
        if (loclvl == 0)
        {
          ...
          assert(enterkeydir);
          outdir2 = outdir->GetDirectory(kname.c_str());
          assert(outdir2);
          outdir2->cd();
          if (jp::debug)
            cout << kname.c_str() << endl;
        }
        else
        {
          if (jp::debug)
            cout << kname.c_str() << " (at bottom)" << endl;
        ...
        else if (loclvl == 0 and TString(indir2->GetName()).Contains("FullEta"))
        {
          ...
          if (inobj)
          {
            if (string(indir2->GetName()) != "FullEta_Gen")
              cout << "FullEta";
            TH1 *hpt = dynamic_cast<TH1 *>(inobj->Clone(hname.c_str()));
            hpt->Reset();
            if (hpt)
        ...
        else
        {
          if (loclvl == 0 or std::find(_ignoretrgs.begin(), _ignoretrgs.end(), indir2->GetName()) == _ignoretrgs.end())
          {
            ...
            {
              if (string(indir2->GetName()) == "jt40" or string(indir2->GetName()) == "mc")
                cout << " ";
              cout << indir2->GetName();
              if (string(indir2->GetName()) != jp::reftrig)
                cout << ",";
            }
    ...
    if (obj)
    {
      ...
      else
      {
        ...
        if (obj->InheritsFrom("TH3"))
          hpt = dynamic_cast<TH3D *>(obj);
        else if (obj->InheritsFrom("TH2"))
          hpt = dynamic_cast<TH2D *>(obj);
