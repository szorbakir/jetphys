...
double unc = 0.01;
double unc_lead = 0.01; // Default for MC
double unc_sublead = 0.01;
...
void HistosFill::PrintMemInfo(bool printcout)
{
  ...
  PrintInfo(Form("MemInfo(Tot:%d, Used:%d, Free:%d, Stot:%d, SUsed:%d, SFree:%d",
                 ...
                 _info.fSwapTotal, _info.fSwapUsed, _info.fSwapFree),
            printcout);
}
...
bool HistosFill::Init(TChain *tree)
{
  ...
  _outfile = new TFile(Form("output-%s-1.root", jp::type), "RECREATE");
...
#endif
    if (jp::ismc)
      fChain->SetBranchStatus(Form("PFJets%s_.partonFlavour_", jp::chs), 1);
  ...
  if (jp::gitinfo)
    gROOT->ProcessLine(".!rm gitlog.txt");
...
void HistosFill::Loop()
{
  ...
  for (Long64_t djentry = 0; djentry < _nentries; djentry += 1 + jp::skim)
  {                             // Event loop
    _jentry = djentry + _nskip; // Add a shift from beginning
    if (LoadTree(_jentry) < 0)
      break;
    nb = fChain->GetEntry(_jentry);
    nbytes += nb;
    ...
    if (testval % hopval == repval)
    { // 1M report (first report timed to be early)
      // Report memory usage to avoid malloc problems when writing file
      PrintInfo(Form("Doing Loop(), %dM events:", int(_jentry / 1e6 + 0.5)));
      PrintMemInfo();
      cout << endl
           << Form("Processed %lld events (%1.1f%%) in %1.0f sec.",
                   _jentry - 1, 100. * djentry / _nentries, stop.RealTime())
           << endl;
      cout << "BGN: ";
      bgn.Print();
      TDatime now;
    ...
    if (jp::debug)
      cout << "Histos are being filled!" << endl;
  ...
  if (jp::doEtaHistos)
    WriteEta();
  if (jp::ismc and jp::doEtaHistos and jp::doEtaHistosMcResponse)
    WriteMC();
  if (jp::isdt and jp::doMpfHistos)
    WriteAll();
  if (jp::doBasicHistos)
    WriteBasic(); // this needs to be last, output file closed
...
bool HistosFill::PreRun()
{
  ...
  *ferr << endl;
  ...
  if (jp::doqglfile)
  {
    ...
    double vpt[npt + 1];
    vpt[0] = 0;
    for (int trigidx = 0; trigidx != npt; ++trigidx)
      vpt[trigidx + 1] = jp::trigranges[trigidx][1];
    const int nqgl = 101;
    double vqgl[nqgl + 1];
    for (int qglidx = 0; qglidx != nqgl + 1; ++qglidx)
      vqgl[qglidx] = 0. + 0.01 * qglidx;
    ...
    for (int ieta = 1; ieta != _h3probg->GetNbinsX() + 1; ++ieta)
    {
      for (int ipt = 1; ipt != _h3probg->GetNbinsY() + 1; ++ipt)
      {
        ...
        cout << sg << endl
             << flush;
        TH1D *hqgl_g = dynamic_cast<TH1D *>(finmc->Get(sg.c_str()));
        assert(hqgl_g);
        ...
        TH1D *hqgl_q = dynamic_cast<TH1D *>(finmc->Get(sq.c_str()));
        assert(hqgl_q);
...
bool HistosFill::AcceptEvent(JME::JetResolutionScaleFactor resolution_sf, JME::JetResolution resolution)
{
  ...
  if (jp::debug)
    cout << "JEC and MET calculation and leading jets info!" << endl;
  ...
  for (int jetidx = 0; jetidx != njt; ++jetidx)
  {
    ...
    /*if (jp::isdt and _jecUnc)
    {
      ...
      unc = _jecUnc->getUncertainty(true);
       ...
       //cout << "JEC Loop" << endl;
       //cout << "Enrty: " << _jentry << " Number of jets: " << njt << " "<< jetidx << ". Jet Pt: " << p4.Pt() << " " << jetidx <<". Jet Eta: " << p4.Eta() << " unc: " << unc << endl;
     }*/
    ...
    if (jp::ismc and jp::doSF)
    {
      ...
      double smearFactor_up = 1.;
      double smearFactor_down = 1.;
      bool _matched = false;
      int index = 0;
      ...
      if (_matched)
      {
        ...
        smearFactor_down = 1 + (sf_down - 1.) * dPt / jtpt[jetidx];
      ...
      else
      {
        ...
        if (sf_up > 1)
        {
          double sigma_up = r * std::sqrt(sf_up * sf_up - 1);
          std::normal_distribution<double> d{0, sigma_up};
          smearFactor_up = 1. + d(gen);
        }
        if (sf_down > 1)
        {
          double sigma_down = r * std::sqrt(sf_down * sf_down - 1);
          std::normal_distribution<double> d{0, sigma_down};
      ...
      if (p4.E() * smearFactor_up < MIN_JET_ENERGY)
        smearFactor_up = MIN_JET_ENERGY / p4.E();

      if (p4.E() * smearFactor_down < MIN_JET_ENERGY)
        smearFactor_down = MIN_JET_ENERGY / p4.E();

      // If everything is fine, smear the reco jet in simulation, need to do sorting jets after smearing...
      p4 *= smearFactor;
      p4_JERup *= smearFactor_up;
      p4_JERdown *= smearFactor_down;
      ...
      jte_JERup[jetidx] = p4_JERup.E();
      jtpt_JERup[jetidx] = p4_JERup.Pt();
      jteta_JERup[jetidx] = p4_JERup.Eta();
      jtphi_JERup[jetidx] = p4_JERup.Phi();
      jty_JERup[jetidx] = p4_JERup.Rapidity();

      jte_JERdown[jetidx] = p4_JERdown.E();
      jtpt_JERdown[jetidx] = p4_JERdown.Pt();
      jteta_JERdown[jetidx] = p4_JERdown.Eta();
      jtphi_JERdown[jetidx] = p4_JERdown.Phi();
      jty_JERdown[jetidx] = p4_JERdown.Rapidity();
    }
    ...
    // JER Up variations sorting
    if (jt3leads_JERup[0] == -1 or jtpt_JERup[jt3leads_JERup[0]] < jtpt_JERup[jetidx])
    {
      jt3leads_JERup[2] = jt3leads_JERup[1];
      jt3leads_JERup[1] = jt3leads_JERup[0];
      jt3leads_JERup[0] = jetidx;
    }
    else if (jt3leads_JERup[1] == -1 or jtpt_JERup[jt3leads_JERup[1]] < jtpt_JERup[jetidx])
    {
      jt3leads_JERup[2] = jt3leads_JERup[1];
      jt3leads_JERup[1] = jetidx;
    }
    else if (jt3leads_JERup[2] == -1 or jtpt_JERup[jt3leads_JERup[2]] < jtpt_JERup[jetidx])
    {
      jt3leads_JERup[2] = jetidx;
    }

    // JER Down variation sorting
    if (jt3leads_JERdown[0] == -1 or jtpt_JERdown[jt3leads[0]] < jtpt_JERdown[jetidx])
    {
      jt3leads_JERdown[2] = jt3leads_JERdown[1];
      jt3leads_JERdown[1] = jt3leads_JERdown[0];
      jt3leads_JERdown[0] = jetidx;
    }
    else if (jt3leads_JERdown[1] == -1 or jtpt_JERdown[jt3leads_JERdown[1]] < jtpt_JERdown[jetidx])
    {
      jt3leads_JERdown[2] = jt3leads_JERdown[1];
      jt3leads_JERdown[1] = jetidx;
    }
    else if (jt3leads_JERdown[2] == -1 or jtpt_JERdown[jt3leads[2]] < jtpt_JERdown[jetidx])
    {
      jt3leads_JERdown[2] = jetidx;
    }
  ...
  // The leading indices with JER up variation
  int u0 = jt3leads_JERup[0];
  int u1 = jt3leads_JERup[1];
  int u2 = jt3leads_JERup[2];

  // The leading indices with JER down variation
  int d0 = jt3leads_JERdown[0];
  int d1 = jt3leads_JERdown[1];
  int d2 = jt3leads_JERdown[2];
    ...
    if (rho > 500.)
    {
      ...
      if (jp::debug)
        cout << Form("\nrun:ev:ls %d:%d:%lld : rho=%1.1f njt=%d npv=%d jtpt0=%1.1f sumet=%1.1f met=%1.1f\n",
                     run, lbn, evt, rho, njt, npv, (njt > 0 ? jtpt[i0] : 0.), metsumet, met)
             << flush;
    }
    else
    {
  ...
  if (jp::ismc)
  {
    ...
    if (jp::debug)
      cout << "Entering PU weight calculation!" << endl;
#ifdef NEWMODE
    if (_pass and (jtgenidx[i0] != -1 or jp::isnu))
      ++_cnt["07mcgenjet"];
    else
      return false;
#endif
    if (jp::domctrigsim and njt > 0)
    {
      ...
      if (_pass)
        ...
        return false; // Leading jet is weak
      ...
      if (_pass)
        ...
        return false; // Bad pu areas with zero weight excluded
    }                 // jp::domctrigsim
    _trigs.insert("mc");
    _wt["mc"] = 1.0;
    if (jp::reweighPU)
  ...
  else if (jp::isdt)
  {
    ...
    if (jp::debug)
    {
      cout << "TriggerDecision_.size()==" << TriggerDecision_.size() << endl
           << flush;
      cout << "_availTrigs.size()==" << _availTrigs.size() << endl
           << flush;
      cout << "_goodTrigs.size()==" << _goodTrigs.size() << endl
           << flush;
    }
#ifndef NEWMODE
...
void HistosFill::Report()
{
  ...
  if (jp::isdt)
    PrintInfo("Note that for DT it is likely that we lose a large percentage of events for the trigger.\n"
...
void HistosFill::InitBasic(string name)
{
  ...
  {
    if (etas[etaidx + 1] > etas[etaidx])
    { // create real bins only
      ...
      TDirectory *etadir = topdir->GetDirectory(etaname);
      assert(etadir);
      etadir->cd();
      ...
      for (unsigned int j = 0; j != triggers.size(); ++j)
      {
        ...
        TDirectory *dir = etadir->GetDirectory(trg);
        assert(dir);
        dir->cd();
...
void HistosFill::FillSingleBasic(HistosBasic *h)
{
  ...
  if (jp::debug)
    cout << Form("Subdirectory Eta_%1.1f-%1.1f/%s", h->etamin, h->etamax, h->trigname.c_str()) << endl;
  if (jp::debug)
  ...
  if (_pass_qcdmet and i0 >= 0 and _jetids[i0] and jtpt[i0] > jp::goodPt)
  { // First leading jet
    if (i1 >= 0 and _jetids[i1] and jtpt[i1] > jp::goodPt)
    { // Second leading jet

      /*if(_jentry == 22398){
    cout << "Entry: " << _jentry << endl;
    cout << "first jet pt up: " << jtpt_JERup[u0] << " second jet pt up: " << jtpt_JERup[u1] << endl;
    cout << "first jet pt nom: " << jtpt[i0] << " second jet pt nom: " << jtpt[i1] << endl;
    cout << "first jet pt down: " << jtpt_JERdown[d0] << " second jet pt down: " << jtpt_JERdown[d1] << endl; 
    }*/
      ...
      if (jp::ismc and jp::doSF)
      {
        ...
        _j1_JERup.SetPtEtaPhiE(jtpt_JERup[u0], jteta_JERup[u0], jtphi_JERup[u0], jte_JERup[u0]);
        _j1_JERdown.SetPtEtaPhiE(jtpt_JERdown[d0], jteta_JERdown[d0], jtphi_JERdown[d0], jte_JERdown[d0]);

        _j2_JERup.SetPtEtaPhiE(jtpt_JERup[u1], jteta_JERup[u1], jtphi_JERup[u1], jte_JERup[u1]);
        _j2_JERdown.SetPtEtaPhiE(jtpt_JERdown[d1], jteta_JERdown[d1], jtphi_JERdown[d1], jte_JERdown[d1]);
      }

      // Get JEC uncertainty
      if (jp::isdt and _jecUnc and jp::doUnc)
      {
        _jecUnc->setJetEta(_j1.Eta());
        _jecUnc->setJetPt(_j1.Pt());
        unc_lead = _jecUnc->getUncertainty(true);

        //cout << "Dijet Loop" << endl;
        //cout << "Enrty: " << _jentry  << "1. Jet Pt: " << _j1.Pt() <<" 1. Jet Eta: " << _j1.Eta() << " unc: " << unc_lead << endl;

        _jecUnc->setJetEta(_j2.Eta());
        _jecUnc->setJetPt(_j2.Pt());
        unc_sublead = _jecUnc->getUncertainty(true);

        //cout << "Enrty: " << _jentry  << "2. Jet Pt: " << _j2.Pt() <<" 2. Jet Eta: " << _j2.Eta() << " unc: " << unc_sublead << endl;
      }

      //Small check on jet pt ordering.
      if (jtpt[i0] <= jtpt[i1] or jtpt[i0] < jp::goodPt or jtpt[i1] < jp::goodPt)
        cout << "Error!! "
             << " Event: " << _jentry << " Leading Pt:  " << jtpt[i0] << " Subleading Pt: " << jtpt[i1] << endl;

      // Calculate JEC up and down variation of the lorentz vectors and mass variations
      double djmassUp = 0.;
      double djmassDown = 0.;
      double upUncChange = 0.;
      double downUncChange = 0.;

      if (jp::doUnc and jp::isdt)
      {
        _j1Up = _j1 * (1 + unc_lead);
        _j1Down = _j1 * (1 - unc_lead);

        _j2Up = _j2 * (1 + unc_sublead);
        _j2Down = _j2 * (1 - unc_sublead);

        djmassUp = (_j1Up + _j2Up).M();
        djmassDown = (_j1Down + _j2Down).M();
      }

      double djmass_JERup = 0.;
      double djmass_JERdown = 0.;
      double uncChangeJERup = 0.;
      double uncChangeJERdown = 0.;
      // Calculate JER up and down variations
      if (jp::ismc and jp::doSF)
      {

        djmass_JERup = (_j1_JERup + _j2_JERup).M();
        djmass_JERdown = (_j1_JERdown + _j2_JERdown).M();
      }
      ...
      if (ymaxdj >= h->etamin and ymaxdj < h->etamax)
      {
        ...
        if (jp::ismc and jp::doSF)
        {
          // Up shifted JER dijet mass hist
          assert(h->hdjmass_JERup);
          h->hdjmass_JERup->Fill(djmass_JERup, _w);

          uncChangeJERup = (djmass_JERup - djmass) / djmass * 100;
          //cout << "up unc. change:" << upUncChange << endl;

          assert(h->puncChangeJERup);
          h->puncChangeJERup->Fill(djmass, uncChangeJERup, _w);

          // Down shifted JER dijet mass hist
          assert(h->hdjmass_JERdown);
          h->hdjmass_JERdown->Fill(djmass_JERdown, _w);

          uncChangeJERdown = (djmass_JERdown - djmass) / djmass * 100;

          assert(h->puncChangeJERdown);
          h->puncChangeJERdown->Fill(djmass, uncChangeJERdown, _w);
        }

        if (jp::doUnc and jp::isdt)
        {
          // Up shifted JEC dijet mass hist
          assert(h->hdjmassUp);
          ...
          assert(h->pupUncChange);
          h->pupUncChange->Fill(djmass, upUncChange, _w);

          //Down shifted JEC mass hist
  ...
  if (jp::ismc)
  {
    if (gen_njt >= 2 and gen_jtpt[0] > jp::goodPt and gen_jtpt[1] > jp::goodPt)
    {
    ...
    for (int j = 0; j != gen_njt; ++j)
    {
      ...
      for (int k = 0; k != njt; ++k)
      {
        ...
        //	double yreco = jty[k];
        bool reco_id = (_pass && _pass_qcdmet && _jetids[k] && jtpt[k] > jp::goodPt); // && _jetids[k] (this condition needs to be discussed more...)
        if (!reco_id)
          continue;
    ...
    if (gen_njt >= 2 and njt >= 2 and jp::doRmatrix and gen_jtpt[0] > jp::goodPt and gen_jtpt[1] > jp::goodPt and jtpt[i0] > jp::goodPt and jtpt[i1] > jp::goodPt)
    {
    ...
    */
      ...
      if ((gen_ymaxdj >= h->etamin and gen_ymaxdj < h->etamax and ymaxdj >= h->etamin and ymaxdj < h->etamax) and
          (reco_id) and
  ...
  /*if (dphi > 2.7)
        { // Back-to-back condition
          if (alpha < 0.1)
          {
            assert(h->hdjmass_u01);
            h->hdjmass_u01->Fill(djmass, _w);
          }
          if (alpha < 0.2)
          {
            assert(h->hdjmass_u02);
            h->hdjmass_u02->Fill(djmass, _w);
          }
          if (alpha < 0.3)
          {
            assert(h->hdjmass_u03);
            h->hdjmass_u03->Fill(djmass, _w);
          }
        }
      ...
      assert(h->hjmass);
      h->hjmass->Fill(_j1.M(), weight);
      h->hjmass->Fill(_j2.M(), weight);
      assert(h->hjmass0);
      h->hjmass0->Fill(_j1.M(), weight);
      h->hjmass0->Fill(_j2.M(), weight);
      if (dphi > 2.7)
      { // Back-to-back condition
        if (alpha < 0.1)
        {
          assert(h->hjmass_u01);
          h->hjmass_u01->Fill(_j1.M(), weight);
          h->hjmass_u01->Fill(_j2.M(), weight);
        }
        if (alpha < 0.2)
        {
          assert(h->hjmass_u02);
          h->hjmass_u02->Fill(_j1.M(), weight);
          h->hjmass_u02->Fill(_j2.M(), weight);
        }
        if (alpha < 0.3)
        {
          assert(h->hjmass_u03);
          h->hjmass_u03->Fill(_j1.M(), weight);
          h->hjmass_u03->Fill(_j2.M(), weight);
        }
      }
      //}*/
  ...
  /*
      if (dphi > 2.7)
      { // Back-to-back condition
        for (auto itag_lead = 0u; itag_lead < 2u; ++itag_lead)
        { // Look for both t&p combos for the leading jets
          ...
          if (fabs(etatag) < 1.3 and fabs(etaprobe) >= h->etamin and fabs(etaprobe) < h->etamax)
          { // Eta sel.
            ...
            if (jp::do3dHistos)
            {
              ...
              assert(h->hdjasymmtp);
              h->hdjasymmtp->Fill(pttag, alpha, asymmtp, _w);
              assert(h->hdjmpf);
              h->hdjmpf->Fill(ptave, alpha, mpf, _w);
              assert(h->hdjmpftp);
              h->hdjmpftp->Fill(pttag, alpha, mpftp, _w);
            }
            assert(h->ppttagptprobe_noa);
            h->ppttagptprobe_noa->Fill(pttag, ptprobe, _w);
            assert(h->h2pttagptprobe_noa);
            h->h2pttagptprobe_noa->Fill(pttag, ptprobe, _w);
            assert(h->ppttagmu_noa);
            h->ppttagmu_noa->Fill(pttag, trpu, _w);
            assert(h->h2pttagmu_noa);
            h->h2pttagmu_noa->Fill(pttag, trpu, _w);

            //} // Dijet balance
            if (alpha >= 0.3)
            {
              assert(h->ppttageff);
              h->ppttageff->Fill(pttag, 0, _w);
            }
            else
            {
              assert(h->ppttagptprobe);
              h->ppttagptprobe->Fill(pttag, ptprobe, _w);
              assert(h->h2pttagptprobe);
              h->h2pttagptprobe->Fill(pttag, ptprobe, _w);
              assert(h->ppttageff);
              h->ppttageff->Fill(pttag, 1, _w);
              assert(h->ppttagmu);
              h->ppttagmu->Fill(pttag, trpu, _w);
              assert(h->h2pttagmu);
              h->h2pttagmu->Fill(pttag, trpu, _w);
              //{ Composition vs pt tag pt
              // Fractions vs pt: we do pt selection later in HistosCombine
              assert(h->pncandtp);
              h->pncandtp->Fill(pttag, jtn[iprobe], _w);
              assert(h->pnchtp);
              h->pnchtp->Fill(pttag, jtnch[iprobe], _w);
              assert(h->pnnetp);
              h->pnnetp->Fill(pttag, jtnne[iprobe] - jtnhe[iprobe], _w);
              assert(h->pnnhtp);
              h->pnnhtp->Fill(pttag, jtnnh[iprobe] - jtnhh[iprobe], _w);
              assert(h->pncetp);
              h->pncetp->Fill(pttag, jtnce[iprobe], _w);
              assert(h->pnmutp);
              h->pnmutp->Fill(pttag, jtnmu[iprobe], _w);

              assert(h->pchftp);
              h->pchftp->Fill(pttag, jtchf[iprobe], _w);
              assert(h->pneftp);
              h->pneftp->Fill(pttag, (jtnef[iprobe] - jthef[iprobe]), _w);
              assert(h->pnhftp);
              h->pnhftp->Fill(pttag, (jtnhf[iprobe] - jthhf[iprobe]), _w);
              assert(h->pceftp);
              h->pceftp->Fill(pttag, jtcef[iprobe], _w);
              assert(h->pmuftp);
              h->pmuftp->Fill(pttag, jtmuf[iprobe], _w);
              assert(h->phhftp);
              h->phhftp->Fill(pttag, jthhf[iprobe], _w);
              assert(h->pheftp);
              h->pheftp->Fill(pttag, jthef[iprobe], _w);
              assert(h->ppuftp);
              h->ppuftp->Fill(pttag, jtbetaprime[iprobe] * jtchf[iprobe], _w);

              assert(h->ppt_probepertag);
              h->ppt_probepertag->Fill(pttag, ptprobe / pttag, _w);
              ...
              { // Tag fires trigger
                ...
                assert(h->hncandtp);
                h->hncandtp->Fill(jtn[iprobe], _w);
                assert(h->hnchtp);
                h->hnchtp->Fill(jtnch[iprobe], _w);
                assert(h->hnnetp);
                h->hnnetp->Fill(jtnne[iprobe] - jtnhe[iprobe], _w);
                assert(h->hnnhtp);
                h->hnnhtp->Fill(jtnnh[iprobe] - jtnhh[iprobe], _w);
                assert(h->hncetp);
                h->hncetp->Fill(jtnce[iprobe], _w);
                assert(h->hnmutp);
                h->hnmutp->Fill(jtnmu[iprobe], _w);
                assert(h->hnhhtp);
                h->hnhhtp->Fill(jtnhh[iprobe], _w);
                assert(h->hnhetp);
                h->hnhetp->Fill(jtnhe[iprobe], _w);
                assert(h->hchftp);
                h->hchftp->Fill(jtchf[iprobe], _w);
                assert(h->hneftp);
                h->hneftp->Fill((jtnef[iprobe] - jthef[iprobe]), _w);
                assert(h->hnhftp);
                h->hnhftp->Fill((jtnhf[iprobe] - jthhf[iprobe]), _w);
                assert(h->hceftp);
                h->hceftp->Fill(jtcef[iprobe], _w);
                assert(h->hmuftp);
                h->hmuftp->Fill(jtmuf[iprobe], _w);
                assert(h->hhhftp);
                h->hhhftp->Fill(jthhf[iprobe], _w);
                assert(h->hheftp);
                h->hheftp->Fill(jthef[iprobe], _w);
                assert(h->hpuftp);
                h->hpuftp->Fill(jtbetaprime[iprobe] * jtchf[iprobe], _w);

                // Fractions vs number of primary vertices
                assert(h->pncandtp_vsnpv);
                h->pncandtp_vsnpv->Fill(npvgood, jtn[iprobe], _w);
                assert(h->pnchtp_vsnpv);
                h->pnchtp_vsnpv->Fill(npvgood, jtnch[iprobe], _w);
                assert(h->pnnetp_vsnpv);
                h->pnnetp_vsnpv->Fill(npvgood, jtnne[iprobe] - jtnhe[iprobe], _w);
                assert(h->pnnhtp_vsnpv);
                h->pnnhtp_vsnpv->Fill(npvgood, jtnnh[iprobe] - jtnhh[iprobe], _w);
                assert(h->pncetp_vsnpv);
                h->pncetp_vsnpv->Fill(npvgood, jtnce[iprobe], _w);
                assert(h->pnmutp_vsnpv);
                h->pnmutp_vsnpv->Fill(npvgood, jtnmu[iprobe], _w);
                assert(h->pnhhtp_vsnpv);
                h->pnhhtp_vsnpv->Fill(npvgood, jtnhh[iprobe], _w);
                assert(h->pnhetp_vsnpv);
                h->pnhetp_vsnpv->Fill(npvgood, jtnhe[iprobe], _w);
                assert(h->pchftp_vsnpv);
                h->pchftp_vsnpv->Fill(npvgood, jtchf[iprobe], _w);
                assert(h->pneftp_vsnpv);
                h->pneftp_vsnpv->Fill(npvgood, (jtnef[iprobe] - jthef[iprobe]), _w);
                assert(h->pnhftp_vsnpv);
                h->pnhftp_vsnpv->Fill(npvgood, (jtnhf[iprobe] - jthhf[iprobe]), _w);
                assert(h->pceftp_vsnpv);
                h->pceftp_vsnpv->Fill(npvgood, jtcef[iprobe], _w);
                assert(h->pmuftp_vsnpv);
                h->pmuftp_vsnpv->Fill(npvgood, jtmuf[iprobe], _w);
                assert(h->phhftp_vsnpv);
                h->phhftp_vsnpv->Fill(npvgood, jthhf[iprobe], _w);
                assert(h->pheftp_vsnpv);
                h->pheftp_vsnpv->Fill(npvgood, jthef[iprobe], _w);
                assert(h->ppuftp_vsnpv);
                h->ppuftp_vsnpv->Fill(npvgood, jtbetaprime[iprobe] * jtchf[iprobe], _w);

                // Fractions vs true pileup
                assert(h->pchftp_vstrpu);
                h->pchftp_vstrpu->Fill(trpu, jtchf[iprobe], _w);
                assert(h->pneftp_vstrpu);
                h->pneftp_vstrpu->Fill(trpu, (jtnef[iprobe] - jthef[iprobe]), _w);
                assert(h->pnhftp_vstrpu);
                h->pnhftp_vstrpu->Fill(trpu, (jtnhf[iprobe] - jthhf[iprobe]), _w);
                assert(h->pceftp_vstrpu);
                h->pceftp_vstrpu->Fill(trpu, jtcef[iprobe], _w);
                assert(h->pmuftp_vstrpu);
                h->pmuftp_vstrpu->Fill(trpu, jtmuf[iprobe], _w);
                assert(h->phhftp_vstrpu);
                h->phhftp_vstrpu->Fill(trpu, jthhf[iprobe], _w);
                assert(h->pheftp_vstrpu);
                h->pheftp_vstrpu->Fill(trpu, jthef[iprobe], _w);
                assert(h->ppuftp_vstrpu);
                h->ppuftp_vstrpu->Fill(trpu, jtbetaprime[iprobe] * jtchf[iprobe], _w);

                if (jp::doPhiHistos)
                {
                  if (etaprobe > 0)
                  {
                    assert(h->pchfpostp_vsphi);
                    h->pchfpostp_vsphi->Fill(phiprobe, jtchf[iprobe], _w);
                    assert(h->pnefpostp_vsphi);
                    h->pnefpostp_vsphi->Fill(phiprobe, (jtnef[iprobe] - jthef[iprobe]), _w);
                    assert(h->pnhfpostp_vsphi);
                    h->pnhfpostp_vsphi->Fill(phiprobe, (jtnhf[iprobe] - jthhf[iprobe]), _w);
                    assert(h->pcefpostp_vsphi);
                    h->pcefpostp_vsphi->Fill(phiprobe, jtcef[iprobe], _w);
                    assert(h->pmufpostp_vsphi);
                    h->pmufpostp_vsphi->Fill(phiprobe, jtmuf[iprobe], _w);
                    assert(h->phhfpostp_vsphi);
                    h->phhfpostp_vsphi->Fill(phiprobe, jthhf[iprobe], _w);
                    assert(h->phefpostp_vsphi);
                    h->phefpostp_vsphi->Fill(phiprobe, jthef[iprobe], _w);
                    assert(h->ppufpostp_vsphi);
                    h->ppufpostp_vsphi->Fill(phiprobe, jtbetaprime[iprobe] * jtchf[iprobe], _w);
                  }
                  else
                  {
                    assert(h->pchfnegtp_vsphi);
                    h->pchfnegtp_vsphi->Fill(phiprobe, jtchf[iprobe], _w);
                    assert(h->pnefnegtp_vsphi);
                    h->pnefnegtp_vsphi->Fill(phiprobe, (jtnef[iprobe] - jthef[iprobe]), _w);
                    assert(h->pnhfnegtp_vsphi);
                    h->pnhfnegtp_vsphi->Fill(phiprobe, (jtnhf[iprobe] - jthhf[iprobe]), _w);
                    assert(h->pcefnegtp_vsphi);
                    h->pcefnegtp_vsphi->Fill(phiprobe, jtcef[iprobe], _w);
                    assert(h->pmufnegtp_vsphi);
                    h->pmufnegtp_vsphi->Fill(phiprobe, jtmuf[iprobe], _w);
                    assert(h->phhfnegtp_vsphi);
                    h->phhfnegtp_vsphi->Fill(phiprobe, jthhf[iprobe], _w);
                    assert(h->phefnegtp_vsphi);
                    h->phefnegtp_vsphi->Fill(phiprobe, jthef[iprobe], _w);
                    assert(h->ppufnegtp_vsphi);
                    h->ppufnegtp_vsphi->Fill(phiprobe, jtbetaprime[iprobe] * jtchf[iprobe], _w);
                  }
                }
              } // Tag fires trigger
              //} Composition vs pt tag pt
    ...
    { // Jet quality stats
      ...
      if (has2)
        h->pr21->Fill(npvgood, has2 ? jtpt[i1] / jtpt[i0] : 0.);
      if (has3)
        h->pr31->Fill(npvgood, has3 ? jtpt[i2] / jtpt[i0] : 0.);
      if (has32)
        h->pr32->Fill(npvgood, has3 ? jtpt[i2] / jtpt[i1] : 0.);
...
*/
  ...
  {
    if (jp::debug)
      cout << "Loop over jet " << jetidx << "/" << njt << endl
           << flush;
    ...
    { // Jet in eta range
      ...
      if (h->ismcdir)
      {
        ...
        if (_pass_qcdmet)
          h->hpt_nojetid_g->Fill(ptgen, _w);
        if (pass_id)
          h->hpt_noevtid_g->Fill(ptgen, _w);
      }
    } // Jet in eta range
    ...
    { // pt visible
      ...
      {                                               // id OK
        ...
        { // Correct jet eta range
          ...
          h->hpt->Fill(pt, _w);
          assert(h->hpt0);
          h->hpt0->Fill(pt, _w); // Finer binning
          // leading jets (1,2,3)
          if (jetidx == i0)
          {
            assert(h->hpt1);
          ...
          assert(h->hpt_pre);
          if (jp::isdt)
            h->hpt_pre->Fill(pt, _w * _prescales[h->trigname][run] / _wt[h->trigname]);
          if (jp::ismc)
          ...
          // JEC monitoring
          assert(h->pjec_l1);
          h->pjec_l1->Fill(pt, jtjes_l1[jetidx], _w);
          assert(h->pjec_l2l3);
          h->pjec_l2l3->Fill(pt, jtjes_l2l3[jetidx], _w);
          assert(h->pjec_res);
          h->pjec_res->Fill(pt, jtjes_res[jetidx], _w);
          ...
          assert(h->pnne);
          h->pnne->Fill(pt, jtnne[jetidx] - jtnhe[jetidx], _w);
          assert(h->pnnh);
          h->pnnh->Fill(pt, jtnnh[jetidx] - jtnhh[jetidx], _w);
          assert(h->pnce);
          h->pnce->Fill(pt, jtnce[jetidx], _w);
          assert(h->pnmu);
          h->pnmu->Fill(pt, jtnmu[jetidx], _w);
          assert(h->pnhh);
          h->pnhh->Fill(pt, jtnhh[jetidx], _w);
          assert(h->pnhe);
          h->pnhe->Fill(pt, jtnhe[jetidx], _w);
          //
          assert(h->pchf);
          h->pchf->Fill(pt, jtchf[jetidx], _w);
          assert(h->pnef);
          h->pnef->Fill(pt, (jtnef[jetidx] - jthef[jetidx]), _w);
          assert(h->pnhf);
          h->pnhf->Fill(pt, (jtnhf[jetidx] - jthhf[jetidx]), _w);
          assert(h->pcef);
          h->pcef->Fill(pt, jtcef[jetidx], _w);
          assert(h->pmuf);
          h->pmuf->Fill(pt, jtmuf[jetidx], _w);
          assert(h->phhf);
          h->phhf->Fill(pt, jthhf[jetidx], _w);
          assert(h->phhf);
          h->phef->Fill(pt, jthef[jetidx], _w);
          assert(h->ppuf);
          h->ppuf->Fill(pt, jtbetaprime[jetidx] * jtchf[jetidx], _w);
          ...
          if (jp::doqglfile)
          { // If we loaded a previous file to _h3probg, use this for a better probg value
            assert(_h3probg);
          ...
          { // MC extras
            ...
            if (partonflavor[jetidx] == 21)
              h->hgpt_g->Fill(ptgen, _w);
            else
              h->hqpt_g->Fill(ptgen, _w);
    ...
    { // Unbiased gen spectrum (for each trigger)
      ...
      if (fabs(etagen) >= h->etamin and fabs(etagen) < h->etamax)
      {
        ...
        { // Only the 'mc' trigger
          ...
          // Ozlem: (gluon vs quark)
          if (gen_partonflavor[gjetidx] == 21)
            h->hgpt_g0->Fill(gen_jtpt[gjetidx], _w);
          else
            h->hqpt_g0->Fill(gen_jtpt[gjetidx], _w);
...
void HistosFill::FillSingleEta(HistosEta *h, Float_t *_pt, Float_t *_eta, Float_t *_phi)
{
  ...
  int i1 = jt3leads[1];
  int i2 = jt3leads[2];
  if (_pass_qcdmet and i0 >= 0 and _jetids[i0] and jtpt[i0] > jp::recopt and i1 >= 0 and _jetids[i1] and jtpt[i1] > jp::recopt)
  { // Quality conditios for leading jets
    ...
    { // Back-to-back condition
      ...
      { // Look for both t&p combos for the leading jets
        ...
        double etaprobe = jteta[iprobe];
        if (fabs(etatag) < 1.3)
        { // Tag required to be in the barrel region
          double pttag = jtpt[itag];
          ...
          if (alpha < 0.3 and pttag >= h->ptmin and pttag < h->ptmax)
          { // Alpha and trigger
            assert(h->pchftp_vseta);
            h->pchftp_vseta->Fill(etaprobe, jtchf[iprobe], _w);
            assert(h->pneftp_vseta);
            h->pneftp_vseta->Fill(etaprobe, (jtnef[iprobe] - jthef[iprobe]), _w);
            assert(h->pnhftp_vseta);
            h->pnhftp_vseta->Fill(etaprobe, (jtnhf[iprobe] - jthhf[iprobe]), _w);
            assert(h->pceftp_vseta);
            h->pceftp_vseta->Fill(etaprobe, jtcef[iprobe], _w);
            assert(h->pmuftp_vseta);
            h->pmuftp_vseta->Fill(etaprobe, jtmuf[iprobe], _w);
            assert(h->phhftp_vseta);
            h->phhftp_vseta->Fill(etaprobe, jthhf[iprobe], _w);
            assert(h->pheftp_vseta);
            h->pheftp_vseta->Fill(etaprobe, jthef[iprobe], _w);
            assert(h->ppuftp_vseta);
            h->ppuftp_vseta->Fill(etaprobe, jtbetaprime[iprobe] * jtchf[iprobe], _w);
...
void HistosFill::FillJetID(vector<bool> &id)
{
  ...
  for (int jetidx = 0; jetidx != njt; ++jetidx)
  {
    ...
    { // 2016 specialities
      ...
      if (etabs > 2.5)
        id[jetidx] = jtidloose[jetidx];
      // TightLepVeto
      if (etabs <= 2.7 and (jtcef[jetidx] >= 0.90 or jtmuf[jetidx] >= 0.80))
        id[jetidx] = false;
    }
    else if (jp::yid == 1)
    { // 2017 specialities
      if (etabs <= 2.7 and (jtcef[jetidx] >= 0.80 or jtmuf[jetidx] >= 0.80))
        id[jetidx] = false;
    }
    else if (jp::yid == 2)
    {
...
bool HistosFill::LoadLumi(const char *filename)
{
  ...
  if (s != "#Data tag : v2 , Norm tag: None")
    return false;
  ...
  while (getline(f, s, '\n'))
  {
    ...
    { // The user should know if this happens, since we can choose to use only STABLE BEAMS
      if (skip)
        cout << "Skipping line (effects the recorded lumi):\n"
             << s << endl;
      skip = false;
      continue;
    }
...
bool HistosFill::LoadPuProfiles(const char *datafile, const char *mcfile)
{
  ...
  if (!f_pudist or f_pudist->IsZombie())
    return false;
  TFile *fpumc = new TFile(mcfile, "READ");
  if (!fpumc or fpumc->IsZombie())
    return false;
  ...
  if (!_pumc)
    return false;
  if (jp::isnu)
  { // In the neutrino gun samples we look at the hardest PU event, so need to shift PU by -1
    ...
    if (jp::isdt)
    {
      ...
      for (auto trigi = 0u; trigi < _availTrigs.size(); ++trigi)
      {
        ...
        *ferr << str;
        ...
        if (trgplace != _goodTrigs.end())
          *ferr << "_y (" << _goodWgts[trgplace - _goodTrigs.begin()] << ") ";
        else
          *ferr << "_n ";
        if (trigi % (jp::notrigs + 1) == jp::notrigs)
          *ferr << endl;
      }
      *ferr << endl
            << flush;
    ...
    else if (jp::pthatbins)
    {
      ...
      if (_pthatrepeats <= 0)
      {
        ...
        PrintInfo(Form("Pthat bin changing.\nFile %d %s, position %lld with %lld events.",
                       sliceIdx, filename.c_str(), centry, noevts),
                  true);
        ...
        while (fChain->GetTreeNumber() < fChain->GetNtrees() - 1)
        {
          ...
          PrintInfo(Form("File extension %d %s, position %lld with %lld events",
                         nextIdx, nextname.c_str(), nentry, nextevts),
                    true);
          noevts += nextevts;
          ++_pthatrepeats;
        }
      ...
      else
      {
        ...
        PrintInfo(Form("Pthat bin remains the same while file is changing.\nFile %s\nWeight: %f",
                       fChain->GetCurrentFile()->GetName(), _pthatweight),
                  true);
      }
    }
...
bool HistosFill::GetTriggers()
{
  ...
  TH1F *usedtrigs = dynamic_cast<TH1F *>(fChain->GetCurrentFile()->Get("ak4/TriggerPass"));
  assert(usedtrigs);
  ...
  for (int trgidx = xax->GetFirst(); trgidx <= xax->GetLast(); ++trgidx)
  {
    ...
    if (trgName.compare("") == 0)
      continue; // Ignore empty places on x-axis
    ...
    if (std::regex_match(trgName, zbs))
    {
      ...
      if (zbcase and utrigs.find(trgName) != utrigs.end())
      {
        ...
        { // No trig era weighting: no relative weights
