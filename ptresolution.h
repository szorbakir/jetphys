...
jer_iov _jer_iov = run2016bcd; //none;
...
TF1 *_fecalpf(0);
double ecalprefire(double pt, double eta, string era) {

  jer_iov run;
  if(era == "RunB" or era == "RunC" or era == "RunD")
    run = run2016bcd;
  if(era == "RunE" or era == "RunF" or era == "RunFe")
    run = run2016ef;
  if(era == "RunFl" or era == "RunG" or era == "RunH")
    run = run2016gh;
  ...
  //{0.091, 233.4, 22.9},pars16bcd // 2016BCD, eta 2.0-2.5
  ...
  //cout << "Pars1 " << (*pars)[iy][0] << endl;
  //cout << "Pars2 " << (*pars)[iy][1] << endl;
  //cout << "Pars3 " << (*pars)[iy][2] << endl;
  //cout << "eta: " << eta << " pt:" << pt << " return value: " << _fecalpf->Eval(pt) << endl;
